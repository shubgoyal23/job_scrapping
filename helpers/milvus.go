...
func PushToMilvus() {
	defer func() {
		if r := recover(); r != nil {
			LogError(fmt.Sprintf("PushToMilvus handler crashed because %s", r), nil)
		}
	}()
	...
	if err != nil {
		LogError("Unable to get data from Redis key milvus_starttime, check logs", err)
		return
	}
	startP, err := strconv.Atoi(startPage)
	if err != nil {
		LogError("Unable to convert milvus_starttime to int, check logs", err)
		return
	}
	...
	for {
		...
		if err != nil {
			LogError("Unable to push data to Milvus, check logs", err)
			break
		}
		resp.Body.Close()
		r.Close()
		...
		if err := SetRedisKeyVal("milvus_startpage", strconv.Itoa(page)); err != nil {
			LogError("Unable to set data in Redis key milvus_starttime, check logs", err)
			return
		}
	}
}
