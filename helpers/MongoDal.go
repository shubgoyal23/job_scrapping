...
func InitMongoDB() error {
	...
	if err != nil {
		LogError("InitMongoDB", "connection mongodb", err)
		return err
	}
	MongoDBConn = client
...
func InsertMongoDB(data types.JobDataScrapeMap) error {
	if err := MongoDBConn.Ping(context.Background(), nil); err != nil {
		LogError("InsertMongoDB", "connection to mongodb failed with error: ", err)
		return err
	}
	collection := MongoDBConn.Database(MongoDBName).Collection("jobsScrapeMap")
	_, err := collection.InsertOne(context.Background(), data)
	if err != nil {
		LogError("InsertMongoDB", "cannot insert in mongodb", err)
		return err
	}
	return nil
}
...
func GetOneDocMongoDB(filter bson.M) (interface{}, error) {
	var res interface{}
	if err := MongoDBConn.Ping(context.Background(), nil); err != nil {
		LogError("GetOneDocMongoDB", "connection to mongodb failed with error: ", err)
		return res, err
	}
	collection := MongoDBConn.Database(MongoDBName).Collection("jobsScrapeMap")
	elem := collection.FindOne(context.Background(), filter)
	if elem.Err() != nil {
		LogError("GetOneDocMongoDB", "cannot get doc in mongodb", elem.Err())
		return res, elem.Err()
	}
	if err := elem.Decode(&res); err != nil {
		LogError("GetOneDocMongoDB", "cannot decode doc in mongodb", err)
		return res, err
	}
	return res, nil
}
