...
type JobListingFeilds struct {
	...
	JobType               string `json:"job_type"`               // Type of job (e.g., Full-time, Part-time, Contract, Internship)
	Location              string `json:"location"`               // Location of the job (city, state, country)
	RemoteOption          string `json:"remote_option"`          // Is the job remote? (true/false)
	SalaryMin             string `json:"salary_min"`             // Minimum salary for the job
	SalaryMax             string `json:"salary_max"`             // Maximum salary for the job
	ExperienceMin         string `json:"experience_min"`         // Minimum experience required (in years)
	ExperienceMax         string `json:"experience_max"`         // Maximum experience required (in years)
	EducationRequirements string `json:"education_requirements"` // Required education qualifications
	Skills                string `json:"skills"`                 // Required skills
	Benefits              string `json:"benefits"`               // Benefits offered by the company
	JobPostingDate        string `json:"job_posting_date"`       // Date and time when the job was posted
	ApplicationDeadline   string `json:"application_deadline"`   // Deadline for applications
	JobURL                string `json:"job_url"`                // URL to the job listing on the website
	CreatedAt             string `json:"created_at"`             // Record creation timestamp
	UpdatedAt             string `json:"updated_at"`             // Record update timestamp
}
